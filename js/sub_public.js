...
const mainNav = () => {
    const header = get('#header');
    const headerLogo = get('#header .logo a img');
    const gnblist = getAll('#header .gnb ul .li-box');
    const gnbul = get('#header .gnb ul');
    const gnbh = getAll('#header .gnb ul li h3 a');
    const gnbmenu = getAll('#header .gnb ul li .nav-content li ');
    const loginbar = get('#header .login');
    const languagebar = get('#header .language');
    const iconcolor = getAll('#header .icon li a i');
    ...
    gnblist.forEach((item, idx) => {
        ...
        item.addEventListener('mouseleave', (e) => {
            ...
            gnbh.forEach((item, idx) => {
                header.addEventListener('mouseleave', (e) => {
                    item.style.color = '#333';
                    headerLogo.setAttribute('src', `images/logo-black.png`);
                });
            });
        });
...
const comInit = () => {
    ...
    getPage('page/sub_header.html', '#header');
